...
async function getPrice( coin )
{
    ...
    // Coin Listing Panel
    document.getElementById( 'price-' + lc_coin ).textContent = Number( prevClosePrice ).toFixed(DP);
    document.getElementById( '24hr-' + lc_coin ).textContent = priceChangePercent + "%";
    document.getElementById( '24hrPriceChange-' + lc_coin ).textContent = Number( priceChange ).toFixed(DP);

    // Coin Listing - More Info
    document.getElementById( 'highPrice-' + lc_coin ).textContent = Number( highPrice ).toFixed(DP);
    document.getElementById( 'lowPrice-' + lc_coin ).textContent = Number( lowPrice ).toFixed(DP);


    //var index = coin.indexOf( 'USDT' );
    var asset = coin.substr( 0, index );
    document.getElementById( 'volume-' + lc_coin ).textContent = Number( volume ).toFixed(2) + ' ' + asset ;
...
async function getPrice( coin )
{
    ...
    // Coin Listing Panel
    document.getElementById( 'price-' + lc_coin ).textContent = prevClosePrice ;
    document.getElementById( '24hr-' + lc_coin ).textContent = priceChangePercent + "%";
    //document.getElementById( '24hrPriceChange-' + lc_coin ).textContent = priceChange;

    // Coin Listing - More Info
    document.getElementById( 'highPrice-' + lc_coin ).textContent = highPrice ;
    document.getElementById( 'lowPrice-' + lc_coin ).textContent = lowPrice ;
